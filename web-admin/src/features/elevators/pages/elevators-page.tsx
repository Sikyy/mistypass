...
export function ElevatorsAdaptedPage({ token, viewer, placeID }: { token: string; viewer: CurrentUser; placeID?: string }) {
  ...
  return (
    <PageFrame
      ...
      actions={
        <Button className="h-11 rounded-[6px] bg-[#4f55ff] px-6 text-white hover:bg-[#3439cc]" onClick={() => setShowCreate(true)}>
    ...
    >
      {showCreate && (
        <div className="mb-6 rounded-[6px] border border-[#eceef2] bg-white p-6">
          <h3 className="mb-3 font-semibold text-[#17171c]">New Elevator</h3>
          <div className="flex gap-3">
            <input
              type="text" placeholder="Elevator name" value={elevatorName}
              onChange={(e) => setElevatorName(e.target.value)}
              className="h-10 flex-1 rounded-[6px] border border-[#d9dbe3] bg-white px-3 text-sm outline-none focus:border-[#8589ff]"
            />
            <Button className="h-10 bg-[#4f55ff] px-5 text-white" disabled={!elevatorName.trim()} onClick={() => createMutation.mutate()}>Create</Button>
      ...
      {elevators.length === 0 ? (
        <div className="rounded-[6px] border border-[#d9dbe3] bg-white px-6 py-12 text-center">
          <Building2Icon className="mx-auto size-8 text-[#9a9ca7]" />
          <p className="mt-3 text-sm font-semibold text-[#17171c]">No elevators</p>
          <p className="mt-1 text-sm text-[#6f717c]">Add an elevator to manage floor access.</p>
        </div>
      ) : (
        elevators.map((elevator) => {
          const elevatorStops = stops.filter((s) => s.elevator_id === elevator.id)
          return (
            <div key={elevator.id} className="mb-4 rounded-[6px] border border-[#eceef2] bg-white">
              <div className="flex items-center gap-4 border-b border-[#eceef2] px-5 py-4">
                <Building2Icon className="size-5 text-[#4f55ff]" />
                <div className="flex-1">
                  <h3 className="font-semibold text-[#17171c]">{elevator.name}</h3>
                  {elevator.description && <p className="text-sm text-[#6f717c]">{elevator.description}</p>}
                </div>
                <span className="text-sm text-[#9a9ca7]">{elevatorStops.length} stops</span>
                <Button variant="outline" size="sm" onClick={() => { setShowCreateStop(elevator.id); setStopName("") }}>
                  <PlusIcon className="mr-1 size-3" /> Stop
                </Button>
                <button type="button" className="size-8 rounded-[6px] text-[#6f717c] hover:bg-[#fbfbfc]" onClick={() => setConfirmDelete({ type: "elevator", id: elevator.id })}>
                  <Trash2Icon className="mx-auto size-4" />
                </button>
              </div>
              {showCreateStop === elevator.id && (
                <div className="flex gap-3 border-b border-[#eceef2] px-5 py-3 bg-[#fbfbfc]">
                  <input type="text" placeholder="Stop name (e.g. Floor 3)" value={stopName} onChange={(e) => setStopName(e.target.value)}
                    className="h-9 flex-1 rounded-[6px] border border-[#d9dbe3] bg-white px-3 text-sm outline-none focus:border-[#8589ff]" />
                  <Button size="sm" className="bg-[#4f55ff] text-white" disabled={!stopName.trim()} onClick={() => createStopMutation.mutate()}>Add</Button>
                  <Button size="sm" variant="outline" onClick={() => setShowCreateStop(null)}>Cancel</Button>
                </div>
              )}
              {elevatorStops.map((stop) => (
                <div key={stop.id} className="flex items-center gap-4 border-b border-[#eceef2] px-5 py-3 last:border-b-0">
                  <span className="flex-1 text-sm font-medium text-[#17171c]">{stop.name}</span>
                  <StatusDot tone={stop.status === "locked_down" ? "error" : "success"} label={stop.status === "locked_down" ? "Locked Down" : "Active"} />
                  ...
                  <button type="button" className="size-7 rounded text-[#6f717c] hover:bg-[#fbfbfc]"
