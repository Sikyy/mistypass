...
function useAlignedMinuteTick(): number {
  ...
  useEffect(() => {
    ...
    }, Math.max(1, 60_000 - (Date.now() % 60_000)))
...
export function AccessPage({ token, viewer, activeSectionOverride }: AccessPageProps) {
  ...
  const nowTick = useAlignedMinuteTick()
  ...
  const baseDataQuery = useQuery({
    queryKey: ["access-base-data", platformViewer ? "platform" : "tenant"],
    queryFn: () =>
      loadAccessBaseData({
        token,
        platformViewer,
  ...
  const topologyQuery = useQuery({
    queryKey: ["access-topology-data", selectedTenantID],
    queryFn: () =>
  ...
  const employeesQuery = useQuery({
    queryKey: ["access-enterprise-employees", selectedTenantID],
  ...
  useEffect(() => {
    ...
    setSummary(
      buildEnterpriseFlowSummary(
        t("accessPage.summary.flowAccepted", {
          flowLabel,
          stageLabel: enterpriseFlowStageLabel(enterpriseFlowContext.stage),
          tenantSuffix: tenantLabel ? t("accessPage.summary.organizationSuffix", { tenantLabel }) : "",
          segmentSuffix: flowSegmentDescriptor ? t("accessPage.summary.segmentSuffix", { flowSegmentDescriptor }) : "",
        })
      )
    )
  ...
  async function submitPolicy(payload: {
    name: string
    scopeType: ScopeType
    ...
    if (!selectedTenantID || !payload.name.trim()) {
      return
    }
    const scopeError = validateScope(payload.scopeType, payload.buildingID, payload.areaID, payload.doorID)
    if (scopeError) {
      setError(scopeError)
      return
    }
    ...
    setSummary("")
    try {
      const scopedBuildingID = payload.scopeType === "all" ? "" : payload.buildingID
      const scopedAreaID = payload.scopeType === "area" || payload.scopeType === "door" ? payload.areaID : ""
      const scopedDoorID = payload.scopeType === "door" ? payload.doorID : ""
      const request = {
        ...
        schedule: payload.schedule.trim() || undefined,
      ...
      if (policyEditID) {
        ...
        setSummary(t("accessPage.summary.policyUpdated", { name: updated.name }))
      } else {
        const created = await createAccessPolicy(token, request)
        setPolicies((current) => [created, ...current])
        setSummary(t("accessPage.summary.policyCreated", { name: created.name }))
      }
      ...
      const message = err instanceof Error ? err.message : t("accessPage.errors.savePolicyFailed")
      setError(message)
    }
  }
  ...
  function applyPolicyStarter(starter: PolicyStarter) {
    setError("")
    setSummary(t("accessPage.summary.policyStarterApplied", { groupName: starter.groupName }))
    setPolicyEditID("")
  ...
  async function applyBatchPolicyStatus(nextStatus: "active" | "draft") {
    ...
    if (targetRows.length === 0) {
      setSummary(t("accessPage.summary.noPoliciesForBatchReview"))
      return
    }
    ...
    try {
      ...
      const targetLabel = nextStatus === "active" ? t("accessPage.labels.policyStatusActive") : t("accessPage.labels.policyStatusDraft")
      setSummary(
        t("accessPage.summary.batchPolicyStatusUpdated", {
          total: settled.length,
          targetLabel,
          success: updatedPolicies.length,
          failedSuffix: failedCount > 0 ? t("accessPage.summary.batchPolicyStatusFailedSuffix", { failedCount }) : "",
        })
      )
      if (nextStatus === "active" && updatedPolicies.length > 0) {
        ...
        setPolicyBatchFlowHint(
          inferredTargetIDs.length > 0
            ? t("accessPage.summary.batchPolicyFlowHintWithTargets", {
                policyCount: updatedPolicies.length,
                targetCount: inferredTargetIDs.length,
              })
            : t("accessPage.summary.batchPolicyFlowHint", { policyCount: updatedPolicies.length })
        )
      }
      if (failedCount > 0) {
        setError(t("accessPage.errors.batchPolicyUpdateFailedPartial", { failedCount }))
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : t("accessPage.errors.batchPolicyStatusFailed")
      setError(message)
    } finally {
  ...
  async function submitGroup(payload: { name: string; description: string }) {
    if (!selectedTenantID || !payload.name.trim()) {
      return
    }
    ...
    try {
      ...
      if (groupEditID) {
        ...
        setSummary(t("accessPage.summary.groupUpdated", { name: updated.name }))
      } else {
        ...
        setSummary(t("accessPage.summary.groupCreated", { name: created.name }))
      }
      ...
      const message = err instanceof Error ? err.message : t("accessPage.errors.saveGroupFailed")
      setError(message)
    }
  }
  ...
  async function createStarterGroups() {
    ...
    try {
      ...
      if (createdGroups.length === 0) {
        setError(t("accessPage.errors.createStarterGroupsFailed"))
        return
      }

      setSummary(
        t("accessPage.summary.starterGroupsCreated", {
          createdCount: createdGroups.length,
          failedSuffix: failedCount > 0 ? t("accessPage.summary.starterGroupsFailedSuffix", { failedCount }) : "",
        })
      )
    } catch (err) {
      const message = err instanceof Error ? err.message : t("accessPage.errors.createStarterGroupsRequestFailed")
      setError(message)
    }
  }
  ...
  async function submitGrant(payload: {
    ...
    if (
      !selectedTenantID ||
      !payload.granteeName.trim() ||
      !payload.granteePhone.trim() ||
      !payload.granteeEmail.trim() ||
    ...
    try {
      ...
      const created = await createTemporaryAccess(token, {
        ...
        grantee_name: payload.granteeName.trim(),
        grantee_gender: payload.granteeGender.trim() || undefined,
        grantee_phone: payload.granteePhone.trim(),
        grantee_email: payload.granteeEmail.trim(),
        mobile_model: payload.mobileModel.trim() || undefined,
        pass_type: payload.passType.trim() || undefined,
        valid_until: payload.validUntil.trim(),
      })
      setGrants((current) => [created, ...current])
      setSummary(t("accessPage.summary.grantCreated", { granteeName: created.grantee_name }))
      ...
      const message = err instanceof Error ? err.message : t("accessPage.errors.createGrantFailed")
      setError(message)
    }
  }
  ...
  function applyGrantStarter(starter: GrantStarter) {
    setError("")
    setSummary(t("accessPage.summary.grantStarterApplied", { title: starter.title }))
    ...
    () =>
      filteredGrantLedger.map((grant) => {
        ...
        return {
          ...
          authorizedAtLabel: new Date(grant.authorized_at || grant.created_at).toLocaleString(i18n.language || undefined),
          ...
          remainingVariant: remaining === t("accessPage.labels.expired") ? "destructive" : "outline",
        }
      }),
    [areaByID, buildingByID, doorByID, filteredGrantLedger, i18n.language, nowTick, t, tenantByID]
  )
  const grantLedgerEmptyState =
    tenantGrants.length === 0
      ? t("accessPage.empty.grants")
      : t("accessPage.empty.grantsFiltered")
  ...
  const directoryRemediationLabel = enterpriseFlowContext
    ? deriveEnterpriseRemediationLabel({
        context: enterpriseFlowContext,
        deactivatedLabel: t("accessPage.labels.deactivatedCleanup"),
        normalLabel: t("accessPage.labels.memberReview"),
      })
    : t("accessPage.labels.memberReview")
  const policyRemediationLabel = enterpriseFlowContext
    ? deriveEnterpriseRemediationLabel({
        context: enterpriseFlowContext,
        deactivatedLabel: t("accessPage.labels.deactivatedCleanup"),
        normalLabel: t("accessPage.labels.memberCarry"),
      })
    : t("accessPage.labels.memberCarry")
  ...
  useEffect(() => {
      ...
      if (!groupEditID) {
        ...
        if (hintedGroup) {
          ...
          setEnterpriseSummary(
            t("accessPage.summary.directoryLocatedGroup", {
              groupName: hintedGroup.name,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberCarriedSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
              directoryRemediationLabel,
            })
          )
          ...
          applyHintedGroupMemberQuery()
          setEnterpriseSummary(
            t("accessPage.summary.directoryLocatedGroupByMember", {
              groupName: hintedMemberGroup.name,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberParenSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
              directoryRemediationLabel,
            })
          ...
          setGroupDescription(enterpriseFlowContext.groupDesc || t("accessPage.summary.defaultGroupDescFromEnterprise"))
          setSelectedGroupMemberIDs(hintedGroupMember ? [hintedGroupMember.id] : [])
          applyHintedGroupMemberQuery()
          setEnterpriseSummary(
            t("accessPage.summary.directoryPrefilledGroupHint", {
              groupNameHint,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberLocatedSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
              directoryRemediationLabel,
            })
          ...
          applyHintedGroupMemberQuery()
          setEnterpriseSummary(
            t("accessPage.summary.directoryPrefilledBySync", {
          ...
          applyHintedGroupMemberQuery()
          setEnterpriseSummary(
            t("accessPage.summary.directoryPrefilledByWorker", { enterpriseWorkerCompactTail })
          )
          ...
          applyHintedGroupMemberQuery()
          setEnterpriseSummary(
            t("accessPage.summary.directoryPrefilledStarter", {
              defaultGroup: starter.defaultGroup,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberCarriedSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
    ...
    if (stage === "policies") {
      ...
      if (!policyEditID) {
        ...
        if (hintedPolicy) {
          ...
          setEnterpriseSummary(
            t("accessPage.summary.policyLocated", {
              policyName: hintedPolicy.name,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberCarriedSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
          ...
          setEnterpriseSummary(
            t("accessPage.summary.policyMatchedRows", {
              matchedCount: policyMatch.matchedRows.length,
              policyName: firstMatchedPolicy.name,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberWithPrefixParenSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
              policyRemediationLabel,
            })
          ...
          setEnterpriseSummary(
            t("accessPage.summary.policyPrefilledStarterByHint", {
              groupName: hintedStarter.groupName,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberWithPrefixParenSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
          ...
          setPolicyStatus("draft")
          setEnterpriseSummary(
            t("accessPage.summary.policyPrefilledName", {
              policyNameHint,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberWithPrefixParenSuffix", { hintedMemberLabel })
                : "",
          ...
          applyPolicyStarter(starter)
          setEnterpriseSummary(
            t("accessPage.summary.policyPrefilledStarter", {
              groupName: starter.groupName,
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberWithPrefixParenSuffix", { hintedMemberLabel })
                : "",
          ...
          setPolicyStatus("draft")
          setEnterpriseSummary(
            t("accessPage.summary.policyLocatedEmpty", {
              hintedMemberSuffix: hintedMemberLabel
                ? t("accessPage.summary.hintedMemberWithPrefixParenSuffix", { hintedMemberLabel })
                : "",
              enterpriseSummaryTail,
        ...
        if (!policyName.trim() && enterpriseFlowContext.workerAlertLevel && !policyNameHint && !hintedStarter && !firstMatchedPolicy) {
          ...
          setPolicyStatus("draft")
          setEnterpriseSummary(
            t("accessPage.summary.policyPrefilledByWorker", { enterpriseWorkerCompactTail })
          )
        }
      }
    }
    ...
    if (stage === "issuance") {
      ...
      setEnterpriseSummary(
        t("accessPage.summary.issuanceStageHint", {
          targetPrefix: targetLabel ? t("accessPage.summary.acceptedTargetPrefix", { targetLabel }) : "",
        })
      )
    }
  ...
  useEffect(() => {
    ...
    if (
      !normalizedSummary ||
      !normalizedSummary.startsWith(buildEnterpriseFlowSummary("")) ||
      normalizedSummary.includes(t("accessPage.labels.segmentHintPrefix"))
    ) {
      return
    }
    const withoutTailPunctuation = normalizedSummary.replace(/[。！？!?]+$/g, "")
    setSummary(
      t("accessPage.summary.summaryWithSegment", {
        withoutTailPunctuation,
        flowSegmentDescriptor,
      })
    )
  }, [enterpriseFlowContext, flowSegmentDescriptor, summary, t])
