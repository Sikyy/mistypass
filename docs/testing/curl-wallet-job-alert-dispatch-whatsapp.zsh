...
function ensure_api_running() {
  ...
  (
    ...
    GOCACHE=/tmp/go-build go run ./cmd/api >/tmp/mp_wallet_job_alert_whatsapp_api.log 2>&1
  ) &
  API_PID="$!"
  ...
  if [[ -f /tmp/mp_wallet_job_alert_whatsapp_api.log ]]; then
    tail -n 80 /tmp/mp_wallet_job_alert_whatsapp_api.log
  fi
  exit 1
}
...
TEMPLATE_PAYLOAD="$(jq -nc \
  ...
  --arg name "Alert Dispatch WhatsApp ${RUN_TAG}" \
  '{tenant_id:$tenant,pass_type:"employee",name:$name,status:"active",actor:"qa.alert.dispatch.whatsapp"}')"
...
QUEUE_PAYLOAD="$(jq -nc \
  ...
  --arg t1 "usr-alert-whatsapp-1-${RUN_TAG}" \
  --arg t2 "usr-alert-whatsapp-2-${RUN_TAG}" \
  '{tenant_id:$tenant,template_id:$template,target_type:"user",target_ids:[$t1,$t2],execution_mode:"queued",actor:"qa.alert.dispatch.whatsapp"}')"
...
INACTIVE_RAW="$(api_with_auth PATCH "/api/v1/wallet/templates/${TEMPLATE_ID}/status?tenant_id=${TENANT_ID}" '{"status":"inactive","actor":"qa.alert.dispatch.whatsapp"}')"
split_response "${INACTIVE_RAW}"
require_http_code "200" "set template inactive"

PROCESS_PAYLOAD="$(jq -nc \
  --arg tenant "${TENANT_ID}" \
  '{tenant_id:$tenant,limit:20,worker_count:2,max_retry:3,base_backoff_ms:10,max_backoff_ms:30,actor:"qa.alert.dispatch.whatsapp.worker"}')"
...
SUB_PAYLOAD="$(jq -nc \
  --arg tenant "${TENANT_ID}" \
  '{tenant_id:$tenant,enabled:true,dlq_alert_threshold:1,window_seconds:600,cooldown_seconds:120,channels:{email:false,whatsapp:true},receiver_groups:["security"],actor:"qa.alert.dispatch.whatsapp"}')"
...
DISPATCH_RAW="$(api_with_auth POST "/api/v1/wallet/jobs/alerts/dispatch" "{\"tenant_id\":\"${TENANT_ID}\",\"actor\":\"qa.alert.dispatch.whatsapp\"}")"
...
WHATSAPP_SENT="$(echo "${HTTP_BODY}" | jq -r '.items | map(select(.channel_results[]? | .channel=="whatsapp" and .status=="sent")) | length')"
if [[ "${DISPATCHED}" -lt 1 || "${FAILED}" -ne 0 || "${WHATSAPP_SENT}" -lt 1 ]]; then
  echo "FAIL dispatch alerts: expected dispatched>=1 failed=0 and whatsapp channel sent"
  echo "${HTTP_BODY}"
  exit 1
fi

echo "PASS: wallet job alert dispatch whatsapp regression complete"
