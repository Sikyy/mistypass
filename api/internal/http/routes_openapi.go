...
func openAPIComponents() map[string]any {
	...
	return map[string]any{
		...
		"schemas": map[string]any{
			...
			"GatewayCertificateRevocation":       openAPIGatewayCertificateRevocationSchema(),
			"GatewayCertificateRevocationCreate": openAPIGatewayCertificateRevocationCreateSchema(),
...
func openAPIExtensionOperationDefinitions() []openAPIOperationDefinition {
	return []openAPIOperationDefinition{
		...
		{Method: http.MethodPatch, Path: "/api/v1/gateways/{gatewayID}/status", OperationID: "updateGatewayStatus", Tag: "Hardware Extensions", Summary: "Update gateway status for operations or certificate revocation.", ExtensionGroup: "legacy_gateway"},
		{Method: http.MethodGet, Path: "/api/v1/gateways/cert-revocations", OperationID: "fetchGatewayCertificateRevocations", Tag: "Hardware Extensions", Summary: "Fetch gateway mTLS certificate serial revocations.", Collection: true, ResponseSchema: "GatewayCertificateRevocation", ExtensionGroup: "legacy_gateway"},
		{Method: http.MethodPost, Path: "/api/v1/gateways/cert-revocations", OperationID: "revokeGatewayCertificateSerial", Tag: "Hardware Extensions", Summary: "Revoke a gateway mTLS certificate serial.", ResponseSchema: "GatewayCertificateRevocation", RequestSchema: "GatewayCertificateRevocationCreate", ExtensionGroup: "legacy_gateway"},
		{Method: http.MethodDelete, Path: "/api/v1/gateways/cert-revocations/{serialNumber}", OperationID: "restoreGatewayCertificateSerial", Tag: "Hardware Extensions", Summary: "Restore a runtime gateway mTLS certificate serial revocation.", ResponseSchema: "GatewayCertificateRevocation", ExtensionGroup: "legacy_gateway", Conflict: true},
...
func openAPIGatewayCertificateRevocationSchema() map[string]any {
	return map[string]any{
		"type":     "object",
		"required": []string{"id", "serial_number", "source"},
		"properties": map[string]any{
			"id":            map[string]any{"type": "string"},
			"tenant_id":     map[string]any{"type": "string"},
			"gateway_id":    map[string]any{"type": "string"},
			...
			"revoked_by":    map[string]any{"type": "string"},
			"revoked_at":    map[string]any{"type": "string", "format": "date-time"},
			"source":        map[string]any{"type": "string", "enum": []string{"runtime", "environment"}},
		},
	}
}

func openAPIGatewayCertificateRevocationCreateSchema() map[string]any {
	return map[string]any{
		"type":     "object",
