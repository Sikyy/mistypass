...
| P2 | OpenAPI client/文档站生成 | 资源字段级 schema 已补齐 20+ 资源（Place/Lock/User/Group/Team/Role/RoleAssignment/Share/Card/Controller/Reader/Terminal/AlertPolicy/HolidayCalendar/TimeWindow），含 enum 约束和 format 标注；collection response 已使用 typed item schema；核心 create 操作已有 request body schema；下一步接文档站或 client 生成 |
...
| Apple Pass production protocol | 真实 Pass Type Certificate 签名、APNs push update | Apple Pass mock provider 已落地：结构完整的 pass.json + manifest + mock 签名 + NFC payload + device registration stub；Google Wallet mock provider 已落地：pass class/object + JWT save link + NFC payload + state update stub；`createPassRecord` 已集成 provider 自动分发 save link 和 NFC payload；流程文档见 `docs/CREDENTIAL-FLOWS.md` | P2 |
